...
DYA Studioを使用しない場合、キーマップの変更やモジュールの構成変更にはファームウェアの書き換えが必要です。

- **[zmk-config-LisM](https://github.com/4mplelab/zmk-config-LisM)**  

:::note[組み立てOPのファームウェアは、DYA Studio対応板を導入済み]
（2026/03販売分から）  
ファームウェアの書き換えなしで、キーマップの変更が行える状態になっています。  
:::
...
import FirmwareReleaseLinks from '../../components/FirmwareReleaseLinks.astro';

<FirmwareReleaseLinks />


## モジュール構成とファームウェア

左右それぞれのモジュールに合ったファームウェアを書き込んでください。左右は独立して選択できます。  
`[c/p]`は`central` または `peripheral`。

| モジュール | 左手側ファームウェア | 右手側ファームウェア |
| :--- | :--- | :--- |
| トラックボール | `lism_left_[c/p]_trackball.uf2` | `lism_right_[c/p]_trackball.uf2` |
| 4wayスティック | `lism_left_[c/p]_4waystick.uf2` | `lism_right_[c/p]_4waystick.uf2` |
| 非トラックボール[^1] | `lism_left_[c/p]_non_trackball.uf2` | `lism_right_[c/p]_non_trackball.uf2` |
...
2.  **ブートローダーモードで起動する**  
    ...
    - 反対側のキーボードの外側1番下のキーを押しながら、ブートローダーモードにしたい側のキーボードの外側1番下キーを押します。  
      ![Bootloaderキーの位置](img/bootloader_key.svg)
...
:::note[初回接続時はロック解除が必要]
セキュリティのため、接続直後のキーボードは操作がロックされています。  
`Default Layer`の**最下段・一番外側のキー**（左右どちらでも可）を長押しして`Device Layer`に切り替え、**右手の親指キー**（鍵アイコン）を押すとロックが解除されます。  
![Unlockキーの位置](img/studio_unlock_key.svg)
:::
...
<details class="details-card">
    ...
    <div class="video-embed">
      <iframe
        src="https://www.youtube.com/embed/vXpV9jgRB1M?start=440"
        title="GitHub Actionsでのビルド 参考動画"
...
3.  **ZMK Studioでの設定**
    ...
    3.  ロックを解除します。（ロック解除方法は[DYA Studio利用時の注意事項](#1-dya-studio)と同様です）
    4.  画面の指示に従い、キーマップを編集します。変更は即座に反映されます。