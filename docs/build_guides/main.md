...
!!! info "基板にはバージョンがあり、若干ビルド方法が異なります"
    基板上にバージョンが印字してあります。  
...
=== "ver 1.1.0"
    ...
    ![内容物](img/main01_v1.1.0.jpg){ width="65%" }
...
!!! note "ケースをご自身で用意される方は、[ケースデータ]({{three_d_data_url}}/case){:target="_blank"}の`LisM_vx.x.x.step`をご使用ください。"
...
=== "ver 1.1.0"
    | 部品 | 数量 | 備考 |
    | :--- | :--- | :--- |
    | 3.7Vリチウムポリマーバッテリー | 2 | 302040 ~ 302030型を推奨<br> (組み立てOP3を選択された方は取り付け済)|
...
| 工具名 | 備考 |
| :--- | :--- |
| はんだペースト | 低温 or 中温 ([参考](https://amzn.asia/d/55VmwUT){:target="_blank"}) |
...
4.  ホットプレートで加熱し、リフローを行います。  
    ...
    !!! warning "ダイオードに当たらないように注意してください"

### 2. 基板の切り離し
1. ペンチなどで折り、左右に分割してください。  
   固い場合は、カッターで軽く切り込みを入れると割りやすくなります。  
   ![基板の分割](img/main-build02-01.jpg){ width="65%" }

1. 分割した部分をやすりで滑らかに整えてください。  
   ![やすりがけ](img/main-build02-02.jpeg){ width="65%" }
...
1. 基板のシルク（印字）に合わせて、すべてのソケットをはんだ付けします。  
   ![キーソケットのはんだ付け](img/main-build03-socket.jpg){ width="65%" }
...
1. XIAOの裏`SW_REST1`へスイッチの両端をはんだ付けしてください。  
   ![リセットスイッチのはんだ付け](img/main-build03-rest.jpeg){ width="65%" }


??? quote "(v1.0.0のみ) 電源スイッチ"  

      1. マスキングテープなどで位置を固定し、裏側から足をはんだ付けします。  
         ![電源スイッチのはんだ付け](img/main-build03-switch.jpg){ width="65%" }
...
1. 同梱のコネクタ付き配線を2cm程度の長さでカットして被覆を剥いてください。  
   ![配線のカット](img/main-build04-01.jpeg){ width="65%" }
2. (必要に応じて)予備はんだを付けてください。  
   ![予備はんだ](img/main-build04-02.jpg){ width="65%" }
3. 収縮チューブを通した後、バッテリーの配線とはんだ付けしてください。
   ![バッテリー配線のはんだ付け](img/main-build04-03.jpg){ width="65%" }  
4. 収縮チューブで絶縁してください。  
   配線の長さは、合計で3cm程度が理想です。  
   ![収縮チューブでの絶縁](img/main-build04-04.jpeg){ width="65%" }
...
4.  組み立てた基板を載せ、バッテリーを接続します。  
    ![基板の配置](img/main-build06-07.jpeg){ width="65%" }
...
6.  トップケースをスイッチ側からひっかけるようにして装着し、ボトムケースにはめ込みます。  
    ![top_case1](img/main-build06-10.jpg){ width="65%" }

7.  最後に、手前のモジュール部分を軽く押さえて固定します。  
    ![top_case3](img/main-build06-12.jpg){ width="65%" }

### 7. FFCケーブル収納
1. 画像を参考に引き出しやすい折り方で収納してください。  
   ![FFCケーブルの収納1](img/main-build07-01.jpeg){ width="65%" }
   ![FFCケーブルの収納2](img/main-build07-02.jpeg){ width="65%" }
